...
#[inline]
fn compute_key_and_nonce(password: &[u8]) -> Result<([u8; AES_256_KEY_LEN], [u8; IV_SIZE])> {
    let mut dig = vec![0; AES_256_KEY_LEN + IV_SIZE];
    let mut key = [0; AES_256_KEY_LEN];
    let mut iv = [0; IV_SIZE];
    Argon2::default().hash_password_into(password, SALT, &mut dig)?;
...
mod serde_feature {
    ...
    pub fn serialize_serde<T: Serialize>(data: &T, password: &[u8]) -> Result<Vec<u8>> {
        serialize(&rmp_serde::to_vec(data)?, password)
    }
